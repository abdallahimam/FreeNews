...
function contain_spaces($input) {
	return strpos($input, ' ');
}
 function validate_form($username, $email, $fullname, $pass1, $pass2) {
	$errors = array();
	$t_username = trim($username);
	$t_fullname = trim($fullname);
	$t_pass1 = trim($pass1);
	$t_pass2 = trim($pass2);
	if (empty($t_username) || contain_spaces($t_username) < 0) {
		$errors[] = 'Username shouldn\'t be empty or contain spaces.';
	}
	if (empty($t_pass1) || contain_spaces($t_pass1) < 0) {
		$errors[] = 'Password shouldn\'t be empty or spaces.';
	}
	if ($t_pass1 != $t_pass2) {
		$errors[] = 'Passwords doesn\'t match.';
	}
	if (empty($t_fullname)) {
		$errors[] = 'Full name shouldn\'t be empty.';
	}
